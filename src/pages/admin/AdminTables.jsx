...
export default function AdminTables() {
  ...
  const create = async (e) => {
    ...
    try {
      const t = await createTable({ number: newNumber.trim(), seats: newSeats })
  ...
  const changeSeats = async (table, seats) => {
    try {
      const updated = await updateTable(table.number, { seats })
      setTables((prev) => prev.map((t) => (t.number === updated.number ? updated : t)))
    } catch (err) {
      setError(err?.response?.data?.message || err.message)
    }
  }
      ...
      <div className="flex items-end justify-between gap-3 flex-wrap">
        <div>
          <span className="eyebrow">
            <Table2 className="h-3.5 w-3.5" /> Tables &amp; QR
          </span>
          <h1 className="section-heading mt-1">Floor plan</h1>
          <p className="text-masala-700 mt-1 text-sm">
            Add tables and generate QR codes that guests scan to start ordering.
          </p>
          <div className="mt-2"><UsageGauge resource="tables" /></div>
        </div>
      ...
      <AnimatePresence>
        {adding && (
          <Overlay onClose={() => setAdding(false)}>
            <h2 className="font-display text-2xl text-masala-900">New table</h2>
        ...
        {qrTable && (
          <QRModal table={qrTable} onClose={() => setQrTable(null)} orgSlug={orgSlug} />
        )}
      </AnimatePresence>
      {bulkSheet && (
        <BulkQrSheet
...
function TableCard({ table, onSeats, onDelete, onQr }) {
  ...
  return (
    ...
    >
      ...
      <div className="relative flex items-center gap-2 text-sm text-masala-700">
        ...
        <label className="flex items-center gap-1.5">
          ...
          <input
            ...
            className="w-14 bg-cream border border-saffron-200 rounded-lg px-2 py-0.5 outline-none"
          />
        </label>
      </div>
...
function QRModal({ table, onClose, orgSlug }) {
  const canvasRef = useRef(null)
  // Org-aware URL: scanning this drops a customer directly on the right
  // restaurant's menu, with the table already set on their session.
  const url = orgSlug
    ? `${customerOrigin}/order/${orgSlug}/${table.number}`
    : `${customerOrigin}/?table=${table.number}`

  const download = () => {
    const canvas = canvasRef.current?.querySelector('canvas')
    if (!canvas) return
    const link = document.createElement('a')
    link.download = `masala-story-table-${table.number}.png`
    ...
    w.document.write(`
      ...
      <body>
        <div class="card">
          ...
          <h1>Scan to order</h1>
          <div class="num">T${table.number}</div>
  ...
  return (
    <Overlay onClose={onClose}>
      <div className="flex items-start justify-between">
        <div>
          <span className="eyebrow">Table {table.number}</span>
          <h2 className="font-display text-2xl text-masala-900 mt-1">
            Scan-to-order QR
          </h2>
          <p className="text-masala-700 text-sm mt-1">
            Print this and place it on the table.
          </p>
        </div>
      </div>
